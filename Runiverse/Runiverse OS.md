...
* 평범한 세계와는 다르게 현실의 운영체제랑 똑같은 구조로 흘러간다.
  ...
  * 커널은 **오너 캐릭터가** 커널을 담당하며 커널의 권한은 절대적이므로 내부적으론 커널보다 권한이 높은 건 존재하지 않는다.
    * 다만 이는 내부의 권한이기에 외부로 넘어가면 가면 커널보다 권한이 높을 수 있다.
...
* 이 운영체제는 삭제 및 종료를 할 수 **없다**.
* 현실 사람 또는 현실 그 자채, 아니면 특정 세계관의 캐릭터 또는 세계관 그 자채를 운영체제로 불러올 수 있다.
  * 오너캐는 이를 ``프로세스``라고 부른다.
  * 오너캐 또한 프로세스로 취급하긴 하다. (다만 운영체제에 종속되지 않는 오너캐의 특성상 일반적인 프로세스랑은 다른 특수 프로세스라 언로드가 이론상으로도 불가능하다.)
  * 아, 운영체제라는 이름의 걸맞게 그 세계에 존재하는 프로그램 조차도 실행할 수 있다.
...
* 기본적으로 총 5개의 스레드가 존재한다.
  ...
  * 프로세스 스레드
    * 로드된 프로세스를 처리하는 스레드이다.
    * 메인 <-> 프로세스 스레드끼리 자주 상호작용한다. (당연하게도 Thread-safe)
  * 명령 스레드
    * 일단 Life-cycle에 포함되긴 하지만, 일반적으론 스레드 동시 접근을 피하기 위해 위의 모든 스레드의 연산이 끝났을때 명령을 처리한다.
...
* 스크랜턴 현실성 닻
  * 커널, 즉 루미의 **모든 능력**은 **현실 조작**을 기반으로 이루어진 능력이기에, 스크랜턴 현실성 닻의 영향 범위 내에서는 모든 능력을 사용할 수 없습니다
없습니다.

  또한 그렇기에 루미가 주로 다른 세계로 갈때 자신의 안전을 보장하기 위해 자주 사용하는 방법인 실제로 이동하지 않고 현실 조작을 통해 일종의 빈틈 없는 홀로그램으로 세계와 상호작용하는 것은, 스크랜턴 현실성 닻의 영향 범위 내에서는 세계와 상호작용하는 것은 물론, 자기의 몸을 띄우는것 조차 **불가능하게 될 것 입니다.**

  그렇기에 스크랜턴 현실성 닻의 영향 범위 내에서 상호작용하고 싶다면, 루미는 **직접 걸어들어가야합니다.**
