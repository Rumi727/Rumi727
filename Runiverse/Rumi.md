...
우선, 루미는 전지전능이 **아니라는 것을** 알아야합니다.\
생각보다 성숙할 것이라고는 했지만, 일단 정신 연령이 어리고 지식의 제한도 있습니다.\
이는 현실 조작에 대항할 수 있게 만들어주는 생각보다 큰 제한이예요!

그렇기에 루미의 현실 조작 능력은, 현실의 사람이 본 적 없는 것을 꿈꿀 수 없는 것 처럼 루미가 모르는 지식으로까지 현실을 조작할 수 없습니다.\
예를 들어서, 루미는 클레를 한번도 본 적이 없을땐 클레를 창조할 수 없습니다. (생각해보면 당연한 이치죠?)\
하지만 누군가가 알려줄 경우에는 어느정도 제한이 풀릴 거예요.\
그러니까, 제가 루미한태 클레를 불러와달라고 했을때는 한번도 본적이 없기에 어떻게 생긴지 알지는 못하겠지만 일단 불러올 수는 있게 됩니다.\
설명하기 어렵긴 한데, 이정도면 충분하다 생각해요.

RuniOS 문서에도 설명했다싶이, 현실 조작을 무력화하기 위해서는\
현실 조작에 대한 저항성을 가지는 설정이 있으면 가능합니다.\
하지만, 너무 광범위할 경우 논리적 모순이 생길 수 있기에 저항성이 인정되지 않아요! (세계 전체가 현실 조작에 대해 저항 능력이 있다거나)
...
꿈과 희망을 박살내버리는 암울한 상황이 닥칠때, **기적이 될 수 있게** 의도한 능력이기 때문이예요!\
