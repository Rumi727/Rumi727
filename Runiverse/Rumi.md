...
디지털 생명체는 아날로그 생명체가 기본적으로 가지지 않는 뇌에서 나오는 전기적 신호를 하나 더 가지고 있어요.\
디지털과 관련된 것을 조종하는 신호인데, 쉽게 설명하면 소프트웨어에 직접 들어갔을 때\
그 소프트웨어를 직접적으로 만질 수 있는 힘이예요!

디지털 생명체가 직접 마우스 좌클릭, 우클릭 같은 상호작용도 뇌에서 나오는 전기적 신호를 통해 조작하는거예요!

물론, 아날로그 생명체라고 그런 전기적 신호를 출력하지 못하는 것은 아니예요.\
인간은 적응의 동물이고, 디지털 세계에 적응하게 되면 디지털을 조작할 수 있는 전기적 신호를 보낼 수 있게 학습해요.\
하지만, 그래도 근본적인 차이가 있기 때문에 디지털 생명체처럼 완전히 원하는데로 자유자재로 다루는것은 정말 어려워요.

루미의 제어판을 만지는 것도, 그 전기적 신호를 출력할 줄 알아야 만질 수 있어요. (제어판은 RuniOS 기능이예요.)\
물론 루미의 제어판은 그런 생명체라도 조작할 수 있게 만든 것이기에, 아주 미약한 신호로도 감지할 수 있게끔 해뒀어요!\
그래서 제어판 조작 난이도는 제일 낮죠!\
디지털 세계에 적응하지 못해도, 생각 만하면 제어판이 나와요!
