...
루미랑 지낸지 너무 오래되서, 내가 어디에서 왔고 무슨 종족이였는지가 기억나지 않아\
루미가 말하기를 나는 아날로그에서 온 생명체라곤 해, 하지만 왜인지 이 이상은 얘기해주지 않아

언제 한번, 제어판으로 내 프로그래밍 데이터를 읽은적이 있어\
내 많은 데이터의 중에 종류가 IEarth인 데이터를 봤는데... 프로그래밍에 관해선 잘 모르겠어서 그게 무슨 데이터 종류인지, IEarth라는 단어가 무엇을 의미하는진 모르겠어
...
것보다... 제어판을 보면 운영체제 내부에 있는 모든 생명체의 신체를 특정한 상태로 고정하여 성장하지 못하게 막아둔 것 같던데, 이건 루미의 취향이려나?
